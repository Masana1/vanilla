...
static void set_connector_full_rgb(int drmfd, uint32_t connector_id)
{
    drmModeConnectorPtr c = drmModeGetConnector(drmfd, connector_id);
    if (!c) return;
    for (int i = 0; i < c->count_props; i++) {
        drmModePropertyPtr prop = drmModeGetProperty(drmfd, c->props[i]);
        if (!prop) continue;
            ...
            if (prop->flags & DRM_MODE_PROP_ENUM) {
                for (int j = 0; j < prop->count_enums; j++) {
                    if (strcmp(prop->enums[j].name, "Full") == 0) {
                        full_val = prop->enums[j].value;
                        break;
                    }
                }
            }
            drmModeConnectorSetProperty(drmfd, connector_id, prop->prop_id, full_val);
...
uint32_t handles[AV_DRM_MAX_PLANES];
    ...
    for (int i = 0; i < desc->nb_objects; i++) {
        ...
        uint32_t handle = 0;
        for (size_t j = 0; j < ctx->handle_cache_count; j++) {
            vanilla_drm_handle_t *h = &ctx->handle_cache[j];
            if (h->fd == fd) {
                handle = h->handle;
                break;
            }
...
for (int i = 0; i < res->count_connectors; i++) {
        set_connector_full_rgb(ctx->fd, res->connectors[i]);

drmModeConnectorPtr conn_ptr = drmModeGetConnector(ctx->fd, res->connectors[i]);
if (conn_ptr && conn_ptr->encoder_id) {
drmModeEncoderPtr enc = drmModeGetEncoder(ctx->fd, conn_ptr->encoder_id);
if (enc && enc->crtc_id) {
...
if (conn_ptr) drmModeFreeConnector(conn_ptr);
if (ret) break;
}
...
// --- Ponts de compatibilité natifs SDL requis par main.c ---
