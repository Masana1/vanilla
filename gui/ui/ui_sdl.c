...
typedef struct vanilla_drm_ctx_t {
    ...
    uint32_t plane_color_range_property;
    int atomic_async;
    uint32_t fb_id;
    int got_fb;
...
static int set_plane_async(vanilla_drm_ctx_t* ctx, uint32_t fb_id)
{
    ...
    if (ret >= 0 && ctx->plane_color_range_property) {
        drmModeAtomicAddProperty(
            req, ctx->plane_id, ctx->plane_color_range_property, 1);
    }
...
int vui_sdl_drm_present(vanilla_drm_ctx_t* ctx, AVFrame* frame)
{
    ...
    if (!ctx->got_plane) {
        ...
        if (found < 0) {
            ...
            ctx->got_plane = 1;
            get_object_property(ctx->fd, ctx->plane_id, DRM_MODE_OBJECT_PLANE,
                "COLOR_RANGE", &ctx->plane_color_range_property);
            if (ctx->plane_color_range_property) {
                drmModeObjectSetProperty(ctx->fd, ctx->plane_id, DRM_MODE_OBJECT_PLANE,
                    ctx->plane_color_range_property, 1);
                vpilog("DRM Plane COLOR_RANGE property set to Full (1)\n");
            }
        }
    }
...
int vui_sdl_drm_initialize(vanilla_drm_ctx_t** c, SDL_Window* window)
{
    *c = NULL;
    ...
    for (int i = 0; i < res->count_connectors; i++) {
        drmModeConnectorPtr conn = drmModeGetConnector(ctx->fd, res->connectors[i]);
        if (conn && conn->encoder_id) {
            drmModeEncoderPtr enc = drmModeGetEncoder(ctx->fd, conn->encoder_id);
            if (enc && enc->crtc_id) {
